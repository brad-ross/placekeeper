...
async function negotiate(connection: ChromeRuntimeConnection): Promise<void> {
  await expect(connection.handle({ type: "hello", protocol: "placekeeper.chrome-runtime", protocolVersion: 2, connectionId }))
    .resolves.toMatchObject({ type: "hello-ack", protocolVersion: 2, connectionId });
}
...
describe("Chrome least-authority native runtime", () => {
  ...
  it("fails closed on skew, mid-port version changes, v1 smuggling, and agent-only methods", async () => {
    ...
    await expect(preflight.handle({ type: "hello", protocol: "placekeeper.chrome-runtime", protocolVersion: 1, connectionId }))
  ...
  it("deduplicates a dropped non-idempotent response across fresh native connections", async () => {
    ...
    const invokeThroughFreshConnection = async (id: string) => {
      ...
      await connection.handle({ type: "hello", protocol: "placekeeper.chrome-runtime", protocolVersion: 2, connectionId: id });

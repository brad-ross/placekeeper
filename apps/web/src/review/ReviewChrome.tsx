...
export function ReviewChrome({
  ...
  const controlsForPresentation = (candidate: ReviewChromePresentation): ReactNode => <>
    {candidate === 'expanded' || candidate === 'zoomCompact' ? historyDirect : historyCompact}
    {candidate === 'navigationCompact' ? navigationCompact : navigationDirect}
    {candidate === 'expanded' ? zoomDirect : zoomCompact}
  </>;
  ...
  const sizingCluster = (candidate: ReviewChromePresentation): ReactNode => <div className="review-chrome__viewer-controls" style={{ display: 'inline-flex', gridColumn: 'auto', gridRow: 'auto', flexWrap: 'nowrap' }}>
    <span className="review-chrome__control-cluster">{candidate === 'expanded' || candidate === 'zoomCompact' ? <><button type="button" title="Undo" className="review-chrome__icon-control"><ReviewIcon name="undo" /></button><button type="button" title="Redo" className="review-chrome__icon-control"><ReviewIcon name="redo" /></button></> : <button type="button" title="Edit history" className="review-chrome__icon-control"><ReviewIcon name="more-horizontal" /></button>}</span>
    ...
    <span className="review-chrome__control-cluster">{candidate === 'expanded' ? <><button type="button" title="Zoom out" className="review-chrome__icon-control"><ReviewIcon name="minus" /></button><button type="button" title="Zoom in" className="review-chrome__icon-control"><ReviewIcon name="plus" /></button><span className="review-chrome__stat">{viewerState.zoomReady ? `${viewerState.zoomPercent}%` : '—%'}</span><button type="button" title="Fit PDF to available width" className="review-chrome__icon-control"><ReviewIcon name="fit-width" /></button></> : <button type="button" title="PDF zoom" className="review-chrome__stat">{viewerState.zoomReady ? `${viewerState.zoomPercent}%` : '—%'}</button>}</span>
  </div>;

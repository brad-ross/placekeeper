...
describe("rebuild observer", () => {
  it("coalesces output event varieties into latest-only candidate epochs", async () => {
    ...
    expect(validate).toHaveBeenCalledWith({ outputPath: "/work/paper.pdf", hostHintToken: "panel-a:3", reason: "watcher" });
    expect(results).toEqual([{
      result: { status: "committed" },
      input: { outputPath: "/work/paper.pdf", hostHintToken: "panel-a:3", reason: "watcher" },
    }]);
  });

  it("uses a host-scoped hint token rather than claiming service ordering", async () => {
    const validate = vi.fn(async (_input: { readonly reason: string }) => ({ status: "same-digest" as const }));
    const observer = new RebuildObserver({
      outputPath: "/work/paper.pdf",
      hostHintPrefix: "window-b:panel-c",
      validate,
      markPossiblyStale: vi.fn(async () => undefined),
    });
    expect(observer.noteFileEvent("/work/paper.pdf")).toBe(1);
    await observer.flush();
    expect(validate).toHaveBeenCalledWith(expect.objectContaining({ hostHintToken: "window-b:panel-c:1" }));
  });
  ...
  it("marks source saves stale and revalidates stale panels on reveal and bounded ticks", async () => {
    ...
    expect(markPossiblyStale).toHaveBeenCalledWith({ hostHintToken: "vscode:1" });
  ...
  it("keeps an older output observation from clearing a newer source save", async () => {
    ...
    expect(results).toEqual([]);
    expect(markPossiblyStale).toHaveBeenCalledWith({ hostHintToken: "vscode:2" });
  });

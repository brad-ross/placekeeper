...
2. Protected Recovery is the acknowledgement boundary for an accepted mutation. The session broker writes the complete next state and synchronization metadata before publishing the new state in memory (`apps/service/src/sessions/session-broker.ts:698-742`).
3. The PDF save coordinator freezes the semantic state, projects it into a complete candidate PDF, verifies it, and independently commits it to the selected Save Destination (`apps/service/src/sessions/session-broker.ts:667-695`, `apps/service/src/saving/pdf-save-coordinator.ts:248-359`).
...
A view-only open has no destination decision. The first attempted annotation stays outside canonical state while the UI opens the destination dialog (`apps/web/src/save/save-state-controller.ts:8-16`, `apps/web/src/app/ProductionReviewApp.tsx:760-775`). Only after destination establishment succeeds does the UI submit that pending command (`apps/web/src/app/ProductionReviewApp.tsx:877-899`).

Destination selection is itself durable state. Establishing or relocating one increments its generation and persists the new destination plus `saving` status on the session's serialized write tail (`apps/service/src/sessions/session-broker.ts:500-577`). Selecting the original does not rewrite an untouched zero-item PDF; selecting a copy immediately requests a save and creates the copy when that save succeeds (`apps/service/src/saving/pdf-save-coordinator.ts:191-233`).
...
Every app annotation carries ordinary visible PDF properties plus a namespaced, versioned `pdfMarkup` envelope containing the stable item ID, the semantic Review Item, and redundant visible projection evidence (`packages/core/src/annotation-projection.ts:47-70`, `packages/core/src/portable-annotation.ts:33-43`, `packages/core/src/portable-annotation.ts:252-265`). The pinned EmbedPDF path persists the annotation ID through `/NM` and custom metadata through `/EPDFCustom`; the checked-in engine patch bounds custom metadata before parsing (`packages/pdf-backends/src/embedpdf-adapter.ts:110-122`, `patches/@embedpdf__engines@2.14.4.patch:7-15`).

Ownership requires redundant agreement. Import accepts an annotation only when its envelope has the supported owner and schema, contains a valid item and projection, has a unique visible ID, and matches the visible page, subtype, contents, author, and geometry (`packages/core/src/portable-annotation.ts:267-295`). Missing metadata is foreign. Malformed or mismatched metadata is invalid, never owned. Size, depth, key-count, string-length, and prototype-key limits bound untrusted metadata (`packages/core/src/portable-annotation.ts:10-14`, `packages/core/src/portable-annotation.ts:73-85`, `packages/core/src/portable-annotation.ts:297-309`).

On rewrite, the backend removes only annotations that pass this ownership test, creates the complete current owned set, and leaves Existing PDF Annotations in place (`packages/pdf-backends/src/embedpdf-adapter.ts:473-502`). It then reopens the candidate and rejects it if foreign annotations changed, requested marks are missing, normal appearances are absent, or portable metadata does not reconstruct the requested items (`packages/pdf-backends/src/embedpdf-adapter.ts:508-549`). The independent verifier also checks page fingerprints, foreign inventory, owned identities, geometry, appearances, and writer evidence before a filesystem commit (`apps/service/src/export/pdf-verifier.ts:114-221`).
...
On restart, only non-clean drafts matching an approved source or target identity are recovery candidates (`apps/service/src/sessions/session-broker.ts:186-214`). Resume validates the immutable source, rebinds an original to a newly approved file capability, and reauthorizes a copy only when its fingerprint still matches. A missing or changed target keeps the semantic work protected but becomes visibly `not-saved` until Retry, Locate, or a new destination succeeds (`apps/service/src/sessions/session-broker.ts:228-304`, `apps/web/src/save/SaveDestinationDialog.tsx:66-76`). Clean desired/saved evidence suppresses a later recovery prompt without relying on browser unload or a Finish action (`apps/service/src/sessions/session-broker.ts:186-195`, `apps/service/src/sessions/session-broker.ts:468-497`).
...
Annotation-local, redundantly validated identity also preserves interoperability. Other PDF readers can display ordinary standards-compatible marks without understanding the private envelope, while this app can reconstruct the semantic Review Items after a move, rename, or transfer. Tests cover each supported kind, reject mismatched or duplicate evidence, and recreate imported state with fresh local history (`packages/core/test/portable-annotation.test.ts:42-69`, `packages/core/test/portable-annotation.test.ts:124-135`, `test/conformance/pdf-writer.conformance.test.ts:105-144`).

No single test layer proves this contract. Core tests prove identity validation, service tests prove save/recovery state transitions, PDF conformance tests prove write/reopen behavior, and browser tests prove the first-command gate and visible status. A manual save-copy/reopen check is especially useful: a Saved label proves only that a write completed, whereas reopening the emitted PDF proves that the annotation survived as recognizable application data (session history).
...
3. After destination capability and generation are persisted, the pending command is submitted once (`apps/web/src/app/ProductionReviewApp.tsx:877-899`).
4. Recovery records the new ReviewState before autosave writes, verifies, and atomically commits the copy (`apps/service/src/sessions/session-broker.ts:715-742`, `apps/service/src/server/http-server.ts:423-430`).

### Rapid edits during an in-flight save

If revision 4 is being rendered when revisions 5 and 6 are accepted, both later commands are already protected in recovery. Their requests set the same queue's `requested` bit rather than creating parallel writers. Revision 4 may commit to the current target, but it cannot make synchronization clean because its revision and digest are no longer desired. The next pass freezes revision 6; revision 5 need never exist as physical PDF bytes (`apps/service/src/saving/pdf-save-coordinator.ts:235-257`, `apps/service/src/sessions/session-broker.ts:468-480`).

### Destination switch during an old save

If a generation-2 copy save is in flight when the user selects the original, destination establishment persists generation 3. The generation-2 candidate fails its commit fence under the session tail and its temporary file is removed. The generation-3 request writes the complete latest state to the original (`apps/service/src/sessions/session-broker.ts:598-619`, `apps/service/src/saving/pdf-save-coordinator.ts:345-347`).

### Reopen, edit, delete, and recover

When an annotated PDF is reopened without private recovery data, valid portable envelopes become a fresh revision-0 ReviewState. If rewriting is allowed, that PDF becomes the active original destination already marked clean (`apps/service/src/sessions/session-broker.ts:320-357`, `apps/service/test/recovery.test.ts:513-539`). Later edits and deletes follow the same complete-state autosave path. If the target moves while work is pending, restart recovery restores the semantic state, reports `not-saved`, and lets the user locate or replace the destination (`apps/service/test/recovery.test.ts:368-403`, `apps/service/test/recovery.test.ts:446-511`).

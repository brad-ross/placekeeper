...
The implementation keeps the PDF full-stage and mounts the review workspace `<aside>` continuously. Open state and presentation are independent: the same aside uses `data-workspace-open` for disclosure and `data-workspace-presentation` for right-drawer versus bottom-sheet geometry (`apps/web/src/review/ReferenceWorkspace.tsx:251-266`). CSS hides and reshapes that same element rather than replacing the review tree (`apps/web/src/app/review-layout-annotations.css:158-256`). The production test carries mount probes and zoom state through right-to-bottom-to-right transitions (`test/acceptance/production-flow.spec.ts:1957-2164`).
...
Keep session validity distinct from geometry settlement. `LatestFrameRequest` coalesces resize, mutation, and transition signals; `ViewerGeometrySettlementAuthority` waits until active surface transitions finish; and geometry revisions plus the shell's `layoutGeneration` retrigger only current framing work (`apps/web/src/pdf/viewer-framing.ts:34-132`, `apps/web/src/review/use-annotation-tray-framing.ts:134-216`, `apps/web/src/review/use-annotation-tray-framing.ts:476-488`). A session token alone cannot prove that the rectangles it is about to consume have settled.
...
Clamp each destination to measured limits and skip automatic movement on axes already owned by the user (`apps/web/src/review/use-annotation-tray-framing.ts:446-467`). The production test derives the exact horizontal overlap and requires that only this amount moves while page width, vertical position, and zoom remain unchanged (`test/acceptance/production-flow.spec.ts:2005-2028`).
...
3. Installed-style browser tests against real EmbedPDF geometry for zoomed oversized pages, temporary runway, right/bottom presentation changes, height-only resizing, user-owned close behavior, document-gesture preservation, stacking and hit testing, and stable mount/zoom state (`test/acceptance/production-flow.spec.ts:843-905`, `test/acceptance/production-flow.spec.ts:1957-2327`). Run the geometry-sensitive scenarios in both Chromium and WebKit.

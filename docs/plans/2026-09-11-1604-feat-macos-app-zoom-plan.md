...
- Final rebuilt native candidate: cold-open at persisted 150% painted the viewer and toolbar before any resize; opening the workspace painted the rail and tray. Double-click selected the visible word at 150%. The native View menu correctly disabled PDF zoom and fit while the save dialog was open and retained app zoom actions. Test dialogs were canceled without saving annotations.
- Still outstanding: native XCTest suite, physical unshifted-equal/non-US keyboard verification, real pinch/active-gesture transitions, complete reference-pane/annotation alignment and recovery-path matrix. The feature is not yet fully verified or shipped.
- Execution stopped at the implementation verification gate. The continuation heartbeat is paused; no simplify/review/shipping stages or PR were performed. Temporary candidate processes were closed; the installed app was not replaced.

### Follow-up native verification — XCTest waived by user

- User explicitly waived XCTest on September 11 and requested native keyboard, gesture, reference, and recovery checks. Missing XCTest is no longer a shipping blocker for this run.
- Native candidate at 125%: opened a search result in References, retained its page-1 context while main PDF stayed on page 2, independently scrolled the reference, and moved References between bottom and right layouts. Command–Minus with reference focus changed main PDF zoom from 126% to 106%. Shift–Command–Equal changed app scale while retaining the reference.
- Temporary harness compiled the unchanged production `AppZoomPolicy.swift` and `RecoveryViewController.swift` against packaged recovery assets, without XCTest. Recovery first paint at 150%, live scaling to 200% and 80%, visible Resume hit testing at 200%, visible Fork hit testing at 80%, failed-decision text and disabled choices, and fresh-controller retry with reenabled choices were observed. This establishes recovery-controller UI behavior, not end-to-end service draft recovery.
- User agreed to perform physical keyboard/pinch checks. Candidate remains running at `/tmp/placekeeper-zoom-candidate-final/Placekeeper.app`, unique bundle ID `local.placekeeper.zoomtest`; private runner state `/tmp/placekeeper-native-run-uvBdip/home`, log `/tmp/placekeeper-zoom-additional-native.log`, runner PID 96851. Main PDF page 1 at 106%, app scale 125%, bottom Reference pane page 1. Do not stop runner until manual check is done.
- Physical unshifted equal, pinch in both panes, and any non-US layout remain pending user results. No automation is needed while awaiting this input.

### User-reported zoom layout fixes

- Replaced the fixed native toolbar with a transparent titlebar accessory whose height follows app scale. Native titlebar heights of 65, 78, and 104 points at 125%, 150%, and 200% respectively kept traffic lights, title, and controls centered. At 80% the bar was approximately 41 points tall. Fullscreen entry/exit restored the layout.
- Context actions now use the complete selection rectangle and prefer space above the selection, then below or beside it. A native selection at 125% placed the palette above the selected text with the intended scaled gap. Three focused geometry regressions failed before implementation and passed afterward.
- The tray backing reserves a bottom scrollbar strip only while horizontal overflow exists. Native bottom References now show continuous gray backing when the PDF fits; the horizontal scrollbar remains available when the PDF is wider. The browser regression reproduced the old 20-pixel gap before the fix.
- Verification before cleanup: 59 focused web tests and 13 browser tests passed; TypeScript and native Swift builds passed. Native candidate checks covered the three fixes. Cleanup reused the placement type and made the native titlebar baseline immutable; the Swift build passed again.
- Current isolated candidate: `/tmp/placekeeper-zoom-layout-fixes/Placekeeper.app`, bundle ID `local.placekeeper.zoomtest`, runner log `/tmp/placekeeper-zoom-layout-native.log`. This supersedes the earlier candidate location. The installed app was not replaced.
- Physical Command–Equal and trackpad pinch checks remain pending user results. XCTest remains waived.

...
- KTD4. **Anchor contextual text actions through the public selection seam and a project-owned caret seam.** (session-settled: user-directed — chosen over a persistent toolbar and current-tool control: text actions exist only while a valid anchor exists or a shortcut is being handled.) Use selection completion for Replace, Delete, and Highlight placement. Use reliable page-text hit testing plus the existing caret-anchor contract for Insert. The pinned selection menu placement is the preferred palette seam; characterize its exact local contract before broad UI work and fall back only to public formatted-selection geometry. Governs R3-R5.
- KTD5. **Handle Page Note through scoped page DOM events and fresh placement authority.** (session-settled: user-directed — chosen over page-edge and top-bar entry: the action belongs to the page location.) Use the page provider's React context-menu event because EmbedPDF 2.14.4 exposes no public context-menu interaction event. Convert the invocation point to canonical PDF space once and suppress the native menu only for a safe page target. The Page Note shortcut uses a current safe point or enters a short-lived placement state; it never reuses an offscreen or stale click. Governs R6-R7, R19, R22.
...
2. Render Replace, Delete, and Highlight from the completed reliable selection placement. Render Insert from a reliable caret produced by page-text hit testing. Clear the surface when the anchor clears, becomes unreliable, or is superseded.
...
4. Let the Page Note shortcut consume a current safe point. Without one, focus the current or nearest visible page and enter a transient keyboard placement state with a visible cursor initialized at the visible page center. Arrow keys move the cursor in canonical page space, page navigation reinitializes it at the new current page's visible center, Enter opens the composer at that point, and Escape cancels and restores focus. Pointer placement may commit the same state on release. Keep the cue transient rather than adding a persistent Page Note button.
...
2. Click a reliable boundary between glyphs and verify one caret anchor, only Insert appears, and typing or activating Insert creates one canonical insertion with the expected adjacent context.
...
6. Invoke Page Note with a current safe point and verify it uses that point. In a keyboard-only session with no prior pointer input, invoke it again, move the visible placement cursor, change pages, press Enter, and verify one note uses the new page's canonical coordinates; repeat with Escape and verify focus restoration and no mutation.
...
5. Extract a normalized `ExistingAnnotationSummary` inventory from `App`. Report discovery through a typed callback, including an explicit `existingAnnotations` input for harnesses; let `ProductionReviewApp` hoist the view-only population and pass it to `ReviewShell` as a group distinct from canonical owned `ReviewItem`s. Preserve source subtype, page, contents, and navigation without adding edit, delete, peek, or owned-mark state or writing source annotations into `ReviewState`.
...
8. Feed discovered and explicit existing annotations through the production and acceptance trees. Verify the callback does not duplicate inventory, both drawer groups are reachable, and only owned items expose edit, delete, peek, or correspondence.

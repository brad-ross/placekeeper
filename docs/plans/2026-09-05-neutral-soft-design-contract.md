...
Link-action popover actions show visible text labels beside their existing glyphs, in the existing action order: Open in References, Follow in this tab (only inside References), Open in main document, and Copy link. A fixed-height, non-focusable destination snippet sits above the actions: a rendered strip of the destination page around the target spot with the destination extent highlighted in the Destination Band tokens, plus the bare destination page numeral; a whole-page destination shows the top of the page with nothing highlighted. The popover keeps its placement, flip, focus, and dismissal behavior, its softer material, and consistent icon weight. This supersedes the earlier rule to keep the icon-forward popover (legible link destinations plan, R4).

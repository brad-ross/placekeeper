...
- Preserve long-text reader/expansion behavior, copy-link actions, and supported editing/removal capabilities. No arbitrary shortening for screenshot aesthetics.
...
| Open in main document | Navigate the main viewer using its existing history and workspace behavior |
| Send to main | Promote the active reference location using current tab-consumption, history, and focus semantics |
| Return to reference | Restore that reference tab's original destination; this is not “return to where I was reading” |
...
- Inspect own/imported annotations, overflow actions, long-text reader, peek, copy-link, and correspondence/focus states.

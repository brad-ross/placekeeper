...
A fixed-height, non-focusable destination snippet sits at the top of the link-action popover: a rendered strip of the destination page around the target spot with the destination extent highlighted in the Destination Band color; a whole-page destination shows the top of the page with nothing highlighted. Below it, one row holds the icon-only actions, left-aligned in the existing order (Open in References, Follow in this tab only inside References, Open in main document, Copy link), and the bare destination page numeral right-aligned in the annotation peek card's page style. Actions keep reliable tooltips, which open below the popover so they never cover the snippet. The popover keeps its placement, flip, focus, and dismissal behavior, its softer material, and consistent icon weight. Destination Bands are a soft blue painted like text highlight marks (mark radius, multiply blending, no edge stripe).
...
The main PDF viewport fills the window below the toolbar. The workspace and bottom References are opaque inset overlays above that viewport; zoomed document content continues behind them. Opening, closing, or docking trays does not shrink the PDF viewport or move its scrollbars into the middle of the window. Opening the workspace performs a one-shot Fit width after the overlay geometry settles. Closing, resizing, and docking preserve the current scale and reading anchor. The workspace ends above the full-width bottom References overlay with the same 12 px spacing as the outer tray margins. At narrow widths the supporting surfaces become vertically arranged overlay sheets with reachable headers and independently scrolling bodies.

Overlay occlusion is distinct from document bounds. Allow sufficient scroll reach to bring text out from under a tray, including the last/rightmost content. Explicit navigation, search results, focused items, and return-to-passage should reveal their target in the unobscured reading area without changing zoom. Opening the workspace requests fitting; opening bottom References alone does not. PDF scrolling and zoom gestures belong to the main viewport; wheel/trackpad activity over tray content scrolls that tray, with no accidental chaining into the document.

Fit width uses the unobscured reading width up to the inner edge of the right fade, including when only the collapsed workspace rail is present. Initial placement and each workspace opening use this same fit. It is not a persistent mode: subsequent manual zoom, resizing, closing, and docking retain the current scale. Zooming from a fitted view can extend the paper underneath the overlays. Validate exact anchoring and scroll reach with a real PDF during implementation; the reference scales illustrative paper content.
...
| Replace / deletion annotation / highlight / insertion | `arrow-right-left` / `trash-2` / `highlighter` / `text-cursor` | Use the requested trash glyph for deletion across annotation flows; retain strikethrough on the source text |
...
Annotation type icons are plain glyphs with no badge, enclosing box, extra dot, or provenance overlay. Use the same shape in the type row, peek, selection action, and editor heading: 16 px in metadata and action/header contexts, with the shared stroke weight and neutral color. Replacement uses two simple opposing arrows instead of the detailed dashed-box replacement glyph. Deletion uses the requested trash glyph, reinforced by struck-through source text; Highlight keeps the recognizable highlighter; a comment does not require another tiny symbol attached to the highlighter. Insertion uses the plain text caret; page notes should use a simple `sticky-note` when that production-only type is shown. Accessible names and existing source/alternative text carry the exact meaning; do not try to encode every distinction in a miniature composite icon.
...
### September 6 interface corrections

The following user-requested refinements supersede the corresponding illustrative reference details:

- Keep first-page authoring stable at scroll zero. Deduplicate unchanged editor placements and measure passage visibility independently of the editor itself.
- Show one icon-only passage return in the composer header, only when the entire rendered passage is outside the usable viewer. Partly visible multiline passages remain visible. Keep Cancel and Keep hover treatments consistent with other secondary controls.
- Clear PDF text selection and its action palette when a PDF link is activated, in both Main and References. Use the trash-can glyph for text deletion throughout review flows.
- Style search results like annotation rows, including metadata, actions, spacing, and selected surfaces.
- Track the workspace fade during its opening transition without requiring another input.
- Open the Agent tooltip toward the right and constrain it to the viewport. Align page and zoom popup right edges with their numeric control groups.
- Keep history controls mounted and temporarily disabled while Back or Forward restores a location, preventing toolbar collapse.
- Show errors as floating notices at the upper left of the review stage, using the neutral design's typography, rounded corners, shadows, and error palette.

Verification includes real-PDF Chromium and WebKit tests for these behaviors, sustained frame sampling for composer and toolbar stability, canonical style comparisons, and the complete visual regression suite.
...
VS Code editor commands enter the shared export and annotation workflows. Export uses the same blocked-state guidance, stale-content confirmation, progress, retry, and completion UI as the document menu; repeated pending commands and remounts cannot repeat the export. Reattach selects the first unresolved annotation, or gives a transient neutral status when none need reattachment. Disconnected panels give actionable reopen guidance. Recovery choices use readable native labels, and the unavailable-webview page uses the shared spacing and typography. Editor-title artwork uses transparent 16px light/dark variants; the extension's app artwork remains canonical.

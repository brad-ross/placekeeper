...
- **KTD2. Stream through a narrow, bounded native protocol.** Use start/chunk/finish/cancel with acknowledgements, immutable transfer identity/disposition, phase-closed schemas, per-transfer and aggregate quotas, and strict duration/origin/state limits. The transfer sink owns partial mode-0600 bytes until fsync and existing-backend structural validation produce an opaque sealed-source handle; the broker atomically persists session/recovery ownership before the sink releases it. Return only a typed browser destination or failure; never proxy general daemon control. (session-settled: user-approved — chosen over one giant message or raw daemon forwarding: it supports large PDFs with least authority.) Instantiates R3-R8.
...
- **Test scenarios:**
  ...
  - A hostile parser stall or memory limit terminates validation, keeps the main service/recovery responsive, removes staging, and triggers fallback after the browser stream drains.

...
export function projectHandoffItem(item: ReviewItem, sourceHint?: SourceHint): HandoffItem {
  ...
  const payload: Record<string, JsonValue> = item.kind === "replace" || item.kind === "insert"
    ? { proposedText: string(item, "proposedText") }
    : item.kind === "highlight"
      ? (typeof item.payload.comment === "string" ? { comment: item.payload.comment } : {})
      : item.kind === "pageNote"

...
<body><main><h1>Third-party notices</h1><p>Embedded PDFium revision: <code>${escapeHtml(pdfiumRevision)}</code></p><h2>Demo paper</h2><p><a href="https://arxiv.org/abs/2312.07520v3">Estimating Counterfactual Matrix Means with Short Panel Data</a>, Lihua Lei and Brad Ross. Licensed under <a href="https://creativecommons.org/licenses/by/4.0/">CC BY 4.0</a>. The demo PDF is an excerpt containing selected pages with remapped links and a filtered outline; page content is unchanged. The reader preserves original page numbering and adds illustrative annotation overlays.</p><pre>${escapeHtml(repositoryNotices)}</pre></main></body></html>\n`;
